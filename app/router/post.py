...
@router.post("/",status_code=status.HTTP_201_CREATED,response_model=schema.Post)# response model will handle date send back to user 
def create_post(post:schema.PostCreate,db:Session=Depends(get_db),user_id:int=Depends(oauth2.get_current_user)): #it will take the body and convert into pydantic model
